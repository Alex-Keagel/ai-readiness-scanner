      ...
      const badPaths = instructions.claims
        ...
        .filter(c => {
          ...
          if (c.sourceFile) {
            ...
            if (sourceDir) {
              const resolvedPath = `${sourceDir}/${claim}`;
              if (allKnownPaths.has(resolvedPath) || [...allKnownPaths].some(p => p.includes(resolvedPath) || resolvedPath.includes(p))) return false;
              // Check grandparent resolution (references/ may be sibling of skill)
              if (sourceDir.includes('/')) {
                const grandParent = sourceDir.substring(0, sourceDir.lastIndexOf('/'));
                const gpResolved = `${grandParent}/${claim}`;
                if ([...allKnownPaths].some(p => p.includes(gpResolved) || gpResolved.includes(p))) return false;
              }
            }
            // Check if claim starts with a relative dir that's a known subdir of the source
            // e.g., "references/ev2_mcp.md" from skill dir that has a "references/" subdir
            if (claim.includes('/') && !claim.startsWith('/')) {
              const claimDir = claim.substring(0, claim.indexOf('/'));
              const resolvedDir = sourceDir ? `${sourceDir}/${claimDir}` : claimDir;
              // If the directory portion resolves to a known path, it's likely valid
              if ([...allKnownPaths].some(p => p.includes(resolvedDir))) return false;
            }
          }
          // Skip output/artifact paths (not source references)
          if (claim.startsWith('/') || claim.includes('artifacts/') || claim.includes('output/')) return false;
          return true;
        })
        .slice(0, 5);
