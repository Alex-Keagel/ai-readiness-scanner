...
export async function runDeepAnalysis(
  ...
  // Inject component/directory paths AND file paths for path validation
  try {
    const fs = require('fs');
    const pathMod = require('path');
    const rootPath = workspaceUri.fsPath;
    const allPaths: string[] = [];
    const SKIP = new Set(['node_modules', '.venv', 'venv', '__pycache__', '.git', 'dist', 'build', 'target', 'coverage']);

    // Recursive scan — no depth limit, collect all dirs and files
    function scanDir(dirPath: string, relPath: string) {
      try {
        ...
        for (var i = 0; i < entries.length; i++) {
          ...
          allPaths.push(entryRel);
          if (entry.isDirectory()) {
            scanDir(pathMod.join(dirPath, entry.name), entryRel);
          }
        }
    ...
    logger.info(`Deep: injected ${allPaths.length} paths for path validation`);
  } catch { /* non-critical */ }
  ...
  const crossRefResult = await crossRef.analyze(instructions, codebase, workspaceUri, projectType);
  ...
  if (context) {
    try {
      ...
      const importGraph = new Map<string, string[]>();
      for (const mod of codebase.modules) {
        const { project } = indexer.separateImports('', mod.path);
    ...
    logger.debug('Deep: dead code detection skipped (no extension context)');
  }
